...
public function updateStatus()
{
    ...
    // Perbarui status pelamar
    $updated = $this->applicantModel->updateApplicantStatus($applicant_id, $status);


    if ($updated) {
        return redirect()->back()->with('message', 'Status pelamar berhasil diperbarui.');
    } else {
        return redirect()->back()->with('error', 'Terjadi kesalahan saat memperbarui status pelamar.');
    }
}
